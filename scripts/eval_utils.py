...
def aarons_metric(prediction, target):
   ...
   """
    return (
        ((target * prediction) + ((1 - target) * (1 - prediction)))
        .detach()
        .cpu()
        .numpy()
        .mean()
...
def absolute_error_best(items):
    ...
    return -(
        items.target.values * np.log(items.modal_response.values)
        + (1 - items.target.values) * np.log(1 - items.modal_response.values)
    ).mean()
